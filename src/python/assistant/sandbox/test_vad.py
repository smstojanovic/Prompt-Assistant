...
boundaries = VAD.get_speech_segments("../../../../test1.wav")
...
# %%
audio_data = np.array(audio_sample, dtype= np.float32)
audio_data = audio_data.flatten() / 32768.0

# %%
boundaries = VAD.get_speech_segments(audio_data)
# %%
VAD.get_speech_segments()
# %%
import torchaudio
# %%
# going to get torchaudio to load from an array
data = torchaudio.load(audio_data)
# %%
import io
bytes_io = io.BytesIO()
...
with wave.open(bytes_io, 'wb') as wav_file:
    ...
    wav_file.writeframes(np.array(audio_sample))

# Reset the BytesIO object's position to the beginning
# %%
bytes_io.seek(0)
# %%
boundaries = VAD.get_speech_segments(bytes_io)
# %%
data = torchaudio.load(bytes_io)
# %%
data
# %%
large_chunk, fs = torchaudio.load(
    bytes_io, frame_offset=1000, num_frames=16000
)
# %%
